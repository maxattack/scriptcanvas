...
static void load_syscall_module(lua_State* L, const char* name, const syscall_t *p) {
	lua_newtable(L);
	while(p->name) {
...
def main():
	with open(sys.argv[1], 'w') as src:
		...
		for module, signatures in MODULES.iteritems():
			...
			src.write("static syscall_t mod_%s_syscalls[] = {\n" % module)
			for name in signatures:
				write_line(src, '{ "%s", syscall_%s_%s  },' % (name, module, name))
			write_line(src, '{ 0, 0 }')
			src.write("};\n\n")

		src.write('void ScriptSystem::Bind(lua_State* L) {\n')
		for module in MODULES:
			write_line(src, 'load_syscall_module(L, "%s", mod_%s_syscalls);' % (module, module))
		src.write('}\n\n')
